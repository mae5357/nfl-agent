...
def extract_prediction_from_agent_response(agent_response):
    ...
    for msg in reversed(messages):
        ...
        if isinstance(msg, AIMessage):
            ...
            if hasattr(msg, "content") and hasattr(msg, "type") and getattr(msg, "type", None) == "ai":
                content = msg.content
            else:
                continue
        ...
        if content:
            try:
                ...
                if "home_team_probability" in prediction and "away_team_probability" in prediction:
                    return prediction
            except (json.JSONDecodeError, TypeError):
                # If content is already a dict, check if it has the right structure
                if isinstance(content, dict):
                    if "home_team_probability" in content and "away_team_probability" in content:
                        return content
                continue
...
def main():
    ...
    try:
        ...
        # Invoke agent
        result = agent.invoke({
            "messages": [{"role": "user", "content": matchup}]
        })
